...
mongoose.connect(config.get('db.url'), {
    ...
    //bufferCommands: false // Не ждать переподключения к базе, сразу возвращать ошибку.
});
...
process.on('SIGINT', function(){
    mongoose.connection.close(function(){
        console.log("MongoDB disconnected due to SIGINT");
        process.exit(0);
    });
});
...
const FailedOffer = mongoose.model('FailedOffer', offerSchema);
...
const updateOffer = async (offer) => {
    const thisDoc = new Offer(offer);
    await thisDoc.updateOffer();
}
...
const addFailedOffers = async (offers = []) => {
    return offers.length ? FailedOffer.insertMany(offers, {
        ordered: false,
        rawResult: true
    })
    .then(result => {
...
const clearRawOffers = async (offers) => {
    ...
    return RawOffer.deleteMany({ _id: { $in: ids }});
}
