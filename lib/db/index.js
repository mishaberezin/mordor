...
const updateOffer = async (offer) => {
    ...
    const baseDoc = await Offer.findById(thisDoc._id).exec();

    if(!baseDoc) {
        return await thisDoc.save();
    } else {

        const mergedObj = mergeDocs(baseDoc, thisDoc);
        // const mergedDoc = new Offer(mergedObj);

        const {_id, ...everythingButId} = mergedObj;


        return await baseDoc.replaceOne(everythingButId).exec();
    }
}
