...
const removeByUniqueKey = offers => uniqBy(offers, "uniqueKey");
...
const mergeOffers = offers => {
  ...
  offers.slice(1).forEach(o => {
    ...
    mergeWith(offer, o, (objValue, srcValue, key) => {
      ...
      if (
        key === "addressMeta" &&
        Object(offer.addressMeta).normal === Object(o.addressMeta).normal
      )
        return objValue;

      if (isEqual(objValue, srcValue) || ["timestamp", "diff"].includes(key))
        return srcValue;
...
const dedup = async offers => {
  ...
  // 1. By uniqueKey
  const offersByKey = Object.values(groupBy(offers, "uniqueKey"));
  ...
  return Promise.all(
    offers.map(offer => {
      ...
      return Promise.all(
        dbOffersAddress.map(offerToCmpare =>
          isSimilarByPhotos(offerToCmpare, offer, maxSimilarPhotos).then(
            isSimilar => {
              if (isSimilar) {
                add.push(offer);
              } else {
                update.push(
                  mergeOffers([dbOffersByKey[extractAddress(offer)], offer])
                );
              }
