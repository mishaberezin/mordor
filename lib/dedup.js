...
module.exports = async offers => {
    ...
    // 1. By uniqueKey
    let offersByKey = Object.values(offers.reduce((acc, o) => {
        if (!acc[o.uniqueKey]) {
            acc[o.uniqueKey] = [o];
        } else {
            acc[o.uniqueKey].push(o);
        }
    ...
    offers = offersByKey.reduce((acc, group) => {
        ...
        group.sort((a, b) => a.parsedTimestamp > b.parsedTimestamp ? 1 : -1);
...
const mergeOffers = (offers) => {
    ...
    offers.slice(1).forEach(o => {
        if (offer.parsedTimestamp > o.parsedTimestamp) return;
        ...
        mergeWith(offer, o, (objValue, srcValue, key) => {
            ...
            if (isEqual(objValue, srcValue) ||
                ['parsedTimestamp', 'diff'].includes(key)) return srcValue;

            if (diff[offer.parsedTimestamp]) {
                diff[offer.parsedTimestamp][key] = objValue;
            } else {
                diff[offer.parsedTimestamp] = {};
                diff[offer.parsedTimestamp][key] = objValue;
            }
