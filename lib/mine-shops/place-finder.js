...
async function run(query, resolve) {
    ...
    await page.goto('https://yandex.ru/maps/213/moscow?z=11&ll=37.620393%2C55.753960', {waitUntil: 'networkidle2'});
    await delay(2000);

    const scrollToLoad = async () => {
        const {height, scroll, hasAddButton} = await page.evaluate(selectors => {
            const height = document.querySelector(selectors.scrollContent).offsetHeight;
    ...
    page.on('response', async res => {
        if (!~await res.url().indexOf('https://yandex.ru/maps/api/search')) return;
