...
const config = require('config').get('robots.realty');
...
class RealtyBot {
    ...
    async init() {
        ...
        // try {
        //     await mainPage.waitForSelector(selectors.notNowButton, { timeout: 5000 });
        //     await mainPage.click(selectors.notNowButton);
        // } catch(e) {}
    ...
    async * offers() {
        ...
        const getLinkHref = async (link) => {
            return await (await link.getProperty('href')).jsonValue();
        }

        const waitNewPage = async (url) => {
