...
async function sendItem(item) {
    console.log('Отправлено в endpoint:', item.serviceId);
}

async function getMetroIds() {
    const metroMapUrl = 'https://www.avito.ru/s/avito/components/metro-map/svg-maps/metro-map-moscow.svg';
    ...
    const metroIds = await page.evaluate(function() {
        return [...new Set(
            Array.from(document.querySelectorAll('[data-st-id]'))
                .map(item => item.attributes['data-st-id'].value)
        )];
    });
