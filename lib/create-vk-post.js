...
// const GROUP_ID = '166351231'; // testing

// const ALBUM_OWNER_ID = "492878432"; // misha's vk id
const ALBUM_ID = "257345748"; // misha's hidden album

const ACCESS_TOKEN =
  "3262834d55202a5d8f0d55baf3625599e56b305be1e95eda25573e4f878e6a565d39152c948cf2559be1e"; // misha's token
...
module.exports = async offer => {
  ...
  const picLocators = await uploadPicturesToVk(offer.photos)
    .then(pictures =>
      pictures
        .map(picture => `photo${picture.owner_id}_${picture.id}`)
        .join(",")
    )
    .catch(console.error);
...
const getDescription = async offer => {
  const desc = await fixSpelling(offer.description.slice(0, 200)).then(
    typograf
  );

  let text = `${offer.metro || ""} / ${new Intl.NumberFormat("ru-RU").format(
    offer.price
  )}₽`;
...
const getVkUploadServer = async (albumId, groupId) =>
  fetch(`${VK_API_URL}photos.getUploadServer?album_id=${albumId}${VK_BASE_Q}`)
    .then(res => res.json())
    .then(res => {
      if (res.error) {
...
const uploadPicturesToVk = async links => {
  const vkUploadServer = (await getVkUploadServer(ALBUM_ID, GROUP_ID))
    .upload_url;

  return Promise.all(
    _.chunk(links.slice(0, 10), 5).map(chunk =>
      uploadSomePicturesToVk({ links: chunk, vkUploadServer })
    )
  )
    .then(_.flatten)
    .catch(console.error);
...
const uploadSomePicturesToVk = async ({ links, vkUploadServer }) => {
  ...
  links.forEach((link, i) => {
    ...
    form.append(
      `file${i}`,
      processPicture(request(link)),
      hasExtHint ? {} : { filename: "file.jpg" }
    );
  });
  ...
  return fetch(vkUploadServer, { method: "post", body: form })
    ...
    .then(res =>
      fetch(
        `${VK_API_URL}photos.save?album_id=${res.aid}&photos_list=${
          res.photos_list
        }&server=${res.server}&hash=${res.hash}${VK_BASE_Q}`
      )
        .then(res => res.json())
...
// Adding border, etc.
const processPicture = stream => stream;
